<template>
  <div class="row text-center d-flex justify-content-center mt-3">
    <h1>Professors list</h1>
    <div class="col-lg-10">
      <div class="table-responsive">
        <table
          class="table table-success table-hover align-middle table-bordered"
        >
          <thead>
            <tr>
              <th scope="col">#</th>
              <th scope="col">ID</th>
              <th scope="col">Picture</th>
              <th scope="col">Name</th>
              <th scope="col">Last name</th>
              <th scope="col">Register</th>
              <th scope="col">Actions</th>
            </tr>
          </thead>
          <tbody class="table-group-divider" id="content">
            <tr v-if="loading">
              <td colspan="7">
                <h3>Loading...</h3>
              </td>
            </tr>
            <tr v-else v-for="(professor, i) in professors" :key="professor.id">
              <td v-text="i + 1"></td>
              <td v-text="professor.document"></td>
              <td>
                <img
                  v-if="professor.picture"
                  :src="professor.picture"
                  alt="Profile picture"
                  style="width: 70px !important"
                  class="img-thumbnail"
                />
                <img
                  v-else="professor.picture"
                  src="https://cdn0.iconfinder.com/data/icons/people-137/513/salesman-256.png"
                  alt="Profile picture"
                  style="width: 70px !important"
                  class="img-thumbnail"
                />
              </td>
              <td v-text="professor.first_name"></td>
              <td v-text="professor.last_name"></td>
              <td
                v-text="
                  new Date(professor.created_at).toLocaleDateString('en-US')
                "
              ></td>
              <td>
                <router-link
                  :to="{ name: 'ProfessorView', params: { id: professor.id } }"
                  class="btn btn-info"
                >
                  <i class="fa-solid fa-eye"></i>
                </router-link>

                &nbsp;
                <router-link
                  :to="{ name: 'ProfessorEdit', params: { id: professor.id } }"
                  class="btn btn-warning"
                >
                  <i class="fa-solid fa-edit"></i>
                </router-link>

                &nbsp;
                <button
                  :to="{ path: 'delete/' + professor.id }"
                  class="btn btn-danger"
                  v-on:click="
                    deleteRecord(
                      professor.id,
                      professor.first_name + ' ' + professor.last_name
                    )
                  "
                >
                  <i class="fa-solid fa-trash"></i>
                </button>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
  </div>
</template>

<script>
import axios from "axios";
import { confirm } from "../../functions";

export default {
  data() {
    return {
      professors: null,
      loading: false,
    };
  },
  mounted() {
    this.getProfessors();
  },
  methods: {
    getProfessors() {
      this.loading = true;
      axios
        .get("http://subjectassignmentsystem.test/api/professors")
        .then((res) => {
          this.professors = res.data;
          this.loading = false;
        });
    },
    deleteRecord(id, name) {
      confirm(
        "http://subjectassignmentsystem.test/api/professors/",
        id,
        "Delete record",
        "Are you sure you want to delete to " + name + " ?"
      )
        .then(() => {
          sendRequest("DELETE", { id: id }, urlWithId, "Deleted successfully!")
            .then(() => {
              showAlert("Professor deleted successfully!", "success");
              this.getProfessors();
            })
            .catch((error) => {
              showAlert(error.message, "error");
            })
            .finally(() => {
              this.loading = false;
            });
        })
        .catch(() => {
          this.loading = false;
        });
    },
  },
};
</script>
